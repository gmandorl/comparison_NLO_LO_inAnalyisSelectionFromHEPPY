...
void DrawSame_and_Save(TCanvas * canv, TH1 * histo1, TH1 * histo2,TLegend * leg,std::string name, std::string nameSample, std::string sampleComparison) {
    canv->cd(1);
    ...
    string ratio_str_highRegion = makeString(ratio_highRegion, 3);
    string error_str_highRegion = makeString(error_highRegion, 2);
    
    
    
    if(sampleComparison.find("NLO_LEAD")!=std::string::npos)  {ratio_str = "NLO/LO="+ratio_str; ratio_str_highRegion = "NLO/LO="+ratio_str_highRegion;}
    if(sampleComparison.find("dati_LEAD")!=std::string::npos) {ratio_str = "dati/LO="+ratio_str; ratio_str_highRegion = "dati/LO="+ratio_str_highRegion;}
    if(sampleComparison.find("dati_NLO")!=std::string::npos)  {ratio_str = "NLO/dati="+ratio_str; ratio_str_highRegion = "NLO/dati="+ratio_str_highRegion;}


    
    float yt2 = 0.15;
    float xt2 = 0.25;
    TLatex * t1 = new TLatex(xt2,yt2+0.05 ,(ratio_str+" +- "+error_str).c_str());
    t1->SetNDC();
    t1->SetTextAlign(22);
    t1->SetTextSize(0.04);
    t1->Draw();
    ...
    TLatex * t2 = new TLatex(xt2+0.07 ,yt2 ,(ratio_str_highRegion+" +- "+error_str_highRegion+" in 110<Mll<145").c_str());
    t2->SetNDC();
    t2->SetTextAlign(22);
    t2->SetTextSize(0.04);
    ...
    if(sampleComparison.find("NLO_LEAD")!=std::string::npos)  {integral1_str = "LO  ="+integral1_str; integral2_str = "NLO ="+integral2_str;}
    if(sampleComparison.find("dati_LEAD")!=std::string::npos) {integral1_str = "LO  ="+integral1_str; integral2_str = "dati="+integral2_str;}
    if(sampleComparison.find("dati_NLO")!=std::string::npos)  {integral1_str = "dati="+integral1_str; integral2_str = "NLO ="+integral2_str;}
    
    TLatex * t3 = new TLatex(xt2+0.5,yt2 ,(integral1_str + " +- " + integralErr1_str).c_str());
    t3->SetNDC();
    t3->SetTextAlign(22);
    t3->SetTextSize(0.04);
    t3->Draw();
    ...
    TLatex * t4 = new TLatex(xt2+0.5,yt2+0.05 ,(integral2_str + " +- " + integralErr2_str).c_str());
    t4->SetNDC();
    t4->SetTextAlign(22);
    t4->SetTextSize(0.04);
    t4->Draw();
...
void macroVariabiliOgniTaglio () {


        std::cout << "I can see here" << std::endl;
    
        std::string sampleComparison = "NLO_LEAD";
//         sampleComparison = "dati_LEAD";
//         sampleComparison = "dati_NLO";
        
        
        
//        TFile * f_NLO = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromHEPPY_Inclusive_NLO.root");
//        TFile * f_LO  = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromHEPPY_Inclusive_LO.root");
        
        
//          TFile * f_NLO = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromHEPPY_highMass_NLO.root");
//          TFile * f_LO  = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromHEPPY_highMass_LO.root");
// //         

        TFile * f_NLO = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromSKIM_highMass_NLO.root");
        TFile * f_LO  = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromSKIM_highMass_LO.root");
        
        if (sampleComparison.find("dati_NLO")!=std::string::npos)   f_LO  = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromSKIM_Data.root");
        if (sampleComparison.find("dati_LEAD")!=std::string::npos)  f_NLO = new TFile ("/gpfs/ddn/cms/user/mandorli/Hmumu/CMSSW_8_0_25/src/macroFolder/comparison_NLO_LO_inAnalyisSelectionFromHEPPY_ROOT/ROOT/testfromSKIM_Data.root");
        
        
        
        
        TTree * tree_NLO = (TTree*) f_NLO->Get("tree");
        TTree * tree_LO  = (TTree*) f_LO->Get("tree");
...
//         std::vector<std::string> cutNames = {"no cut", "#jet > 1", "#mu = 2", "mll > 110", "mll < 145", "|eta_qq| >2.5", "pT_1q >35", "pT_2q >25", "Mqq > 200", "pT_1mu >30", "pT_2mu >10", "|eta_1mu| <2.4", "|eta_2mu| <2.4", " bTag < 0.95 ", "preselection (z*<2.5 is the last cut)"};
        std::vector<std::string> cutNames = {"no cut", "#jet > 1", "#mu = 2", "mll > 110", "mll < 145", "pT_1q >35", "pT_2q >25", "pT_1mu >30", "pT_2mu >10", "|eta_1mu| <2.4", "|eta_2mu| <2.4", " bTag < 0.95 ", "z*<2.5", "|eta_qq| >2.5", "preselection (Mqq > 200 is the last cut)"};
        ...
        for(int n = 0; n < nCut; n++) {
            ...
            TH1F * hMll_NLO = new TH1F (("hMll_NLO"+n_str).c_str(), (cutNames[n]+";Mll [GeV];").c_str(), 30, mll_min, mll_max); 
            TH1F * hMll_LO = new TH1F (("hMll_LO"+n_str).c_str(), (cutNames[n]+";Mll [GeV];").c_str(), 30, mll_min, mll_max); 
            hMll_NLO->Sumw2();
            hMll_LO->Sumw2();
            ...
            TH1F * hpt1q_NLO = new TH1F (("hpt1q_NLO"+n_str).c_str(), (cutNames[n]+";pt(1q) [GeV];").c_str(), 30, ptMin, ptMax); 
            TH1F * hpt1q_LO  = new TH1F (("hpt1q_LO"+n_str).c_str(), (cutNames[n]+";pt(1q) [GeV];").c_str(), 30, ptMin, ptMax); 
            hpt1q_NLO->Sumw2();
            hpt1q_LO->Sumw2(); 
            
            
            TH1F * hpt2q_NLO = new TH1F (("hpt2q_NLO"+n_str).c_str(), (cutNames[n]+";pt(2q) [GeV];").c_str(), 30, ptMin, ptMax); 
            TH1F * hpt2q_LO  = new TH1F (("hpt2q_LO"+n_str).c_str(), (cutNames[n]+";pt(2q) [GeV];").c_str(), 30, ptMin, ptMax); 
            hpt2q_NLO->Sumw2();
            hpt2q_LO->Sumw2();
        ...
        float recoJet_pt1_NLO=0;
        float recoJet_pt1_LO=0;
        float recoJet_pt2_NLO=0;
        float recoJet_pt2_LO=0;
...
//         tree_NLO->SetBranchAddress("recoJet_pt1", &recoJet_pt1);
        tree_NLO->SetBranchAddress("recoJet_pt1", &recoJet_pt1_NLO);
        tree_NLO->SetBranchAddress("recoJet_pt2", &recoJet_pt2_NLO);
...
//         tree_LO->SetBranchAddress("GenJet_pt", &pt_q_LO);
        tree_LO->SetBranchAddress("recoJet_pt1", &recoJet_pt1_LO);
        tree_LO->SetBranchAddress("recoJet_pt2", &recoJet_pt2_LO);
...
//            if (recoJet_Mjj_NLO < 175) continue;
...
//                    if(recoJet_Mjj_NLO < 200 && n >0) continue;
//                    if(recoMu_Mll_NLO < 110 && n >3) continue;
...
//                     hVEC_Mll_NLO[n]->Fill(GenMll_NLO, w);
                    ...
                    hVEC_pt1q_NLO[n]->Fill(recoJet_pt1_NLO, w);
                    hVEC_pt2q_NLO[n]->Fill(recoJet_pt2_NLO, w);
...
//                     hVEC_pt1q_NLO[nCut-1]->Fill(recoJet_pt1_NLO, w);
//                     hVEC_pt2q_NLO[nCut-1]->Fill(recoJet_pt2_NLO, w);  
...
//            if (recoJet_Mjj_LO < 175) continue;
...
//                    if(recoJet_Mjj_LO < 200 && n >0) continue;
//                      if(recoMu_Mll_LO < 110 && n >3) continue;
...
//                     hVEC_Mll_LO[n]->Fill(GenMll_LO, w);            
                    ...
                    hVEC_pt1q_LO[n]->Fill(recoJet_pt1_LO, w);
                    hVEC_pt2q_LO[n]->Fill(recoJet_pt2_LO, w);
...
//                     hVEC_pt1q_LO[nCut-1]->Fill(recoJet_pt1_LO, w);
//                     hVEC_pt2q_LO[nCut-1]->Fill(recoJet_pt2_LO, w);  
//                     hVEC_nrecoJet_NLO[nCut-1]->Fill(nrecoJet_NLO, w);
//                     hVEC_genJet_Ht_NLO[nCut-1]->Fill(GenJet_Ht_NLO, w);
//             }
...
//         if(nameSample.find("Inclusive")!=std::string::npos) {
        if (sampleComparison.find("NLO")!=std::string::npos)   myLegend->AddEntry(hVEC_Mll_NLO[0], (nameSample+" NLO").c_str(),"l");
        if (sampleComparison.find("LEAD")!=std::string::npos)  myLegend->AddEntry(hVEC_Mll_LO[0], (nameSample+" LO").c_str(),"l");
            
        if (sampleComparison.find("dati_NLO")!=std::string::npos)   myLegend->AddEntry(hVEC_Mll_LO[0], (nameSample+" dati").c_str(),"l");  
        if (sampleComparison.find("dati_LEAD")!=std::string::npos)  myLegend->AddEntry(hVEC_Mll_NLO[0], (nameSample+" dati").c_str(),"l");  
//         }
...
//         
        ...
        TH1F * countPos_NLO = (TH1F*)f_NLO->Get("CountWeighted");
        TH1F * countNeg_NLO = (TH1F*)f_NLO->Get("countNeg");
        TH1F * countWeighted_NLO = (TH1F*)f_NLO->Get("CountWeighted");
        float TotalNumberOfEvent_NLO = countWeighted_NLO->GetBinContent(1);
        
        TH1F * countPos_LO = (TH1F*)f_LO->Get("CountWeighted");
        TH1F * countNeg_LO = (TH1F*)f_LO->Get("countNeg");
        TH1F * countWeighted_LO = (TH1F*)f_LO->Get("CountWeighted");
        float TotalNumberOfEvent_LO = countWeighted_LO->GetBinContent(1);
...
//         if(nameSample.find("highMass")!=std::string::npos) {
//         if (sampleComparison.find("NLO")!=std::string::npos) normalization_NLO = 42.73/TotalNumberOfEvent_NLO*35900.;
//         if (sampleComparison.find("LEAD")!=std::string::npos)  normalization_LO  = 41.25/TotalNumberOfEvent_LO*35900.;
//         
//         if (sampleComparison.find("dati_LEAD")!=std::string::npos) normalization_NLO = 1.;
//         if (sampleComparison.find("dati_NLO")!=std::string::npos)  normalization_LO  = 1.;
//         }
//         
    
        
        if(nameSample.find("Inclusive")!=std::string::npos) {
        if (sampleComparison.find("NLO")!=std::string::npos) normalization_NLO = 1./TotalNumberOfEvent_NLO*35900.;
        if (sampleComparison.find("LEAD")!=std::string::npos)  normalization_LO  = 1./TotalNumberOfEvent_LO*35900.;
        ...
        if(nameSample.find("highMass")!=std::string::npos) {
        if (sampleComparison.find("NLO")!=std::string::npos) normalization_NLO = 41.81/TotalNumberOfEvent_NLO*35900.;
        if (sampleComparison.find("LEAD")!=std::string::npos)  normalization_LO  = 41.25/TotalNumberOfEvent_LO*35900.;
        }
        ...
        for(int n = 0; n < nCut; n++) {
            ...
            DrawSame_and_Save(canv, hVEC_Mll_LO[n], hVEC_Mll_NLO[n], myLegend, ("figureOgniTaglio/hMll_"+n_str).c_str(), nameSample, sampleComparison);
//             DrawSame_and_Save(canv, hVEC_genMqq_LO[n], hVEC_genMqq_NLO[n], myLegend, ("figureOgniTaglio/hMqqGen_"+n_str).c_str(), nameSample, sampleComparison);
            DrawSame_and_Save(canv, hVEC_recoMqq_LO[n], hVEC_recoMqq_NLO[n], myLegend, ("figureOgniTaglio/hMqqReco_"+n_str).c_str(), nameSample, sampleComparison);
            DrawSame_and_Save(canv, hVEC_pt1q_LO[n], hVEC_pt1q_NLO[n], myLegend, ("figureOgniTaglio/hpt1q_"+n_str).c_str(), nameSample, sampleComparison);
            DrawSame_and_Save(canv, hVEC_pt2q_LO[n], hVEC_pt2q_NLO[n], myLegend, ("figureOgniTaglio/hpt2q_"+n_str).c_str(), nameSample, sampleComparison);
            DrawSame_and_Save(canv, hVECqqDeltaEta_LO[n], hVECqqDeltaEta_NLO[n], myLegend, ("figureOgniTaglio/hqqDeltaEta_"+n_str).c_str(), nameSample, sampleComparison);
            DrawSame_and_Save(canv, hVEC_nrecoJet_LO[n], hVEC_nrecoJet_NLO[n], myLegend, ("figureOgniTaglio/hnrecoJet_"+n_str).c_str(), nameSample, sampleComparison);
//             DrawSame_and_Save(canv, hVEC_genJet_Ht_LO[n], hVEC_genJet_Ht_NLO[n], myLegend, ("figureOgniTaglio/hgenJetHt_"+n_str).c_str(), nameSample, sampleComparison);
